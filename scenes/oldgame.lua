...
function AddNewPlayer(forceColor,keepStats,netid)
    ...
    if netid then
        player.data.netinfo = {id = netid, displayName = Net.UserInfo[netid]}
    end
        ...
        if player then
            ...
            while i <= #Dice do
                if Dice[i].die.doneRolling and not Dice[i].applied then
                    ...
                    if Dice[i].operation == "add" or Dice[i].operation == "mul" then
                        beep("good_dice", 1046.5022612023945, 0, 8, 0.25*Settings["Audio"]["Sound Volume"]/100)
                    else
                        boom("bad_dice", 32, 0.01, 8, 0.25*Settings["Audio"]["Sound Volume"]/100)
                    end
                end
            ...
            while e <= #Entities do
                if Entities[e].hp <= 0 then
                    ...
                    boom("kill", 2, 0.005, 4, 0.5*Settings["Audio"]["Sound Volume"]/100)
                    e = e - 1
                end
                e = e + 1
            end
